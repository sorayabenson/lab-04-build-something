...
const defaultQueries = {
  selectUser(email) {
    return pool.query(`
      SELECT id, email, hash, user_name
      FROM users
      WHERE email = $1;
  `,
    [email]
  ...
  insertUser(user, hash) {
    return pool.query(`
      INSERT into users (email, hash, user_name)
      VALUES ($1, $2, $3)
      RETURNING id, email, user_name;
  `,
    [user.email, hash, user.user_name]
    ).then(result => result.rows[0]);
  }

  //for the future: make updateUser and getUserById
  //figure out how to integrate these with authroutes
};
